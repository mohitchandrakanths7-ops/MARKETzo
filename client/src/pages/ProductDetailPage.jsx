    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
        ...
        <div className="lg:col-span-6 space-y-4 sticky top-28">
          ...
          {/* Main Large Image */}
          <div className="relative aspect-square w-full rounded-3xl overflow-hidden bg-slate-100 border border-slate-200 shadow-lg group">
            <img
              ...
              {images.map((img, i) => (
                <button
                  ...
                  className={`relative w-20 h-20 rounded-2xl overflow-hidden border-2 transition-all shrink-0 ${
                    activeImage === i ? 'border-indigo-600 ring-2 ring-indigo-500/20' : 'border-slate-200 opacity-70 hover:opacity-100'
                  }`}
                >
                  <img
                    src={img}
        ...
        <div className="lg:col-span-6 space-y-6">
          ...
          <div>
            <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-slate-500 mb-2">
              <span className="font-bold text-indigo-600 uppercase tracking-widest">{product.brand?.name || 'Original Edition'}</span>
              <div className="flex items-center gap-2">
                <div className="flex items-center gap-1 text-slate-500">
                  <Store className="w-3.5 h-3.5 text-slate-400" />
                  <span>Sold by <strong className="text-slate-800">{product.seller?.storeName || 'Marketzo Verified Merchant'}</strong></span>
                </div>
                {product.seller?.phone && (
                  <div className="flex items-center gap-1.5 ml-1">
                    <a
                      ...
                      className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-emerald-50 text-emerald-700 hover:bg-emerald-100 font-bold text-[10px] transition-colors"
                    ...
                    <a
                      ...
                      className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-slate-100 text-slate-700 hover:bg-slate-200 font-bold text-[10px] transition-colors"
          ...
          <div className="p-5 bg-slate-50 rounded-3xl border border-slate-200/80 space-y-2">
            <div className="flex items-baseline gap-3">
              ...
              {product.discountPercent > 0 && (
                <span className="px-2.5 py-0.5 rounded-md bg-rose-100 text-rose-700 text-xs font-extrabold">
            ...
            <div className="p-4 bg-gradient-to-br from-amber-50 to-orange-50 rounded-3xl border border-amber-200 shadow-2xs space-y-3">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-slate-900">Wholesale Volume Pricing</span>
                </div>
                <button
                  onClick={() => setIsRfqOpen(true)}
                  className="text-[11px] font-extrabold text-indigo-600 hover:text-indigo-800 underline cursor-pointer text-left"
                >
              ...
              <div className="grid grid-cols-3 gap-2 text-center">
                <div className="bg-white p-2.5 rounded-2xl border border-slate-200 shadow-2xs">
                  <div className="text-[10px] text-slate-500 font-semibold">1 – {(product.moq || 5) - 1} units</div>
                  <div className="text-xs font-black text-slate-900">{formatPrice(product.price)}</div>
                ...
                <div className="bg-white p-2.5 rounded-2xl border border-amber-300 shadow-2xs">
                  <div className="text-[10px] text-amber-700 font-bold">{product.moq || 5} – 49 units</div>
                  <div className="text-xs font-black text-indigo-600">{formatPrice(product.price * 0.90)}</div>
                  <div className="text-[9px] text-emerald-600 font-bold">Save 10%</div>
                </div>
                <div className="bg-white p-2.5 rounded-2xl border border-amber-400 shadow-2xs bg-amber-50/50">
                  <div className="text-[10px] text-amber-800 font-bold">50+ units</div>
                  <div className="text-xs font-black text-indigo-600">{formatPrice(product.price * 0.75)}</div>
                  <div className="text-[9px] text-emerald-600 font-bold">Save 25%</div>
                </div>
              </div>
            </div>
          </div>
          ...
          {product.variants?.length > 0 && (
            <div className="space-y-3">
              <label className="text-xs font-extrabold text-slate-900 uppercase tracking-wider block">
                Select Option: <span className="text-indigo-600">{selectedVariant?.value}</span>
              </label>
              <div className="flex flex-wrap gap-2.5">
                {product.variants.map(v => {
                  ...
                  return (
                    ...
                    >
                      <span>{v.value}</span>
                      {v.priceDiff > 0 && <span className="text-amber-400 ml-1.5">+{formatPrice(v.priceDiff)}</span>}
                    </button>
                  );
                })}
              </div>
          ...
          <div className="flex items-center gap-6 pt-2">
            <div>
              ...
              <div className="flex items-center border border-slate-200 rounded-2xl overflow-hidden bg-white shadow-sm">
                <button
                  ...
                  className="p-2.5 hover:bg-slate-100 text-slate-600"
                >
                  <Minus className="w-4 h-4" />
                </button>
                <span className="px-4 text-xs font-extrabold text-slate-900 min-w-[32px] text-center">
                  {quantity}
                </span>
                <button
            ...
            <div>
              <label className="text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5 block">Stock Status</label>
              <div className="flex items-center gap-1.5 text-xs font-extrabold text-emerald-600 bg-emerald-50 px-3.5 py-2 rounded-2xl border border-emerald-200">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 pt-2">
            <button
              ...
              className="w-full py-4 bg-slate-900 hover:bg-slate-800 text-white font-extrabold text-xs uppercase tracking-wider rounded-2xl transition-all shadow-md flex items-center justify-center gap-2 cursor-pointer"
            >
            ...
            <button
              ...
              className="w-full py-4 bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-500 hover:to-indigo-600 text-white font-extrabold text-xs uppercase tracking-wider rounded-2xl transition-all shadow-lg shadow-indigo-200 flex items-center justify-center gap-2 cursor-pointer"
            >
            ...
            <form onSubmit={handleCheckPincode} className="flex gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3.5 py-2 border border-slate-200 rounded-xl text-xs font-semibold outline-none focus:border-indigo-600 bg-slate-50"
                />
              </div>
              <button
                type="submit"
          ...
          {/* Contact Seller / Merchant Card (Alibaba-style Direct Inquiry) */}
          {/* Contact Seller / Merchant Card (Alibaba-style Direct Inquiry + In-App Chat) */}
          {product.seller && (
            <div className="p-5 bg-gradient-to-br from-slate-900 via-slate-900 to-indigo-950 text-white rounded-3xl border border-slate-800 shadow-xl space-y-4">
                ...
                <div className="flex items-center gap-3.5">
                  ...
                  <div>
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <h4 className="font-black text-sm text-white">{product.seller.storeName}</h4>
              ...
              <div className="text-[11px] text-slate-300 bg-white/5 px-3.5 py-2 rounded-xl flex flex-wrap items-center justify-between gap-2 border border-white/5">
                <span className="flex items-center gap-2">
                  ...
                  <span>Typically responds in <strong className="text-white font-bold">under 10 mins</strong></span>
                </span>
                {product.seller.phone && (
              ...
              {product.seller.phone && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-1">
                  ...
                  <a
                    href={getTelUrl(product.seller.phone)}
                    className="flex items-center justify-center gap-2 py-2.5 px-4 bg-slate-800/90 hover:bg-slate-700 text-white rounded-2xl font-black text-xs uppercase tracking-wider transition-all border border-slate-700/80 shadow-md cursor-pointer active:scale-98"
          ...
          <div className="space-y-4 pt-2">
            <h3 className="font-extrabold text-sm text-slate-900 uppercase tracking-wider">Key Highlights</h3>
      ...
      {product.specs && Object.keys(product.specs).length > 0 && (
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm space-y-4">
          <h3 className="font-extrabold text-base text-slate-900 uppercase tracking-wider">Technical Specifications</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
            {Object.entries(product.specs).map(([key, val]) => (
              <div key={key} className="flex p-3 rounded-2xl bg-slate-50 border border-slate-100 justify-between">
      ...
      {product.frequentlyBought?.length > 0 && (
        <div className="bg-gradient-to-r from-indigo-900 via-slate-900 to-indigo-950 text-white rounded-3xl p-6 sm:p-8 shadow-xl space-y-6">
          <div>
            <span className="text-[11px] font-black uppercase tracking-widest text-amber-400">Bundle & Save 15%</span>
            <h3 className="text-xl font-black tracking-tight text-white mt-1">Frequently Bought Together</h3>
          </div>
          ...
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
            <div className="flex flex-wrap items-center gap-4">
              ...
              <div className="flex items-center gap-3 bg-white/10 p-3 rounded-2xl border border-white/10">
                ...
                <div>
                  <div className="text-xs font-bold text-white max-w-[150px] truncate">{product.name}</div>
              ...
              {product.frequentlyBought.slice(0, 2).map((item, i) => (
                <React.Fragment key={item.id}>
                  <div className="flex items-center gap-3 bg-white/10 p-3 rounded-2xl border border-white/10">
                    ...
                    <div>
                      <div className="text-xs font-bold text-white max-w-[150px] truncate">{item.name}</div>
